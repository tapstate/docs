...
same session to read the server's current self-description:

```console
tapstate(127.0.0.1:8080)> version
cli    0.5.0
server 0.5.0 (127.0.0.1:8080)
dsl    tapstate/v1
data   1
```

When connected, `version` reports the CLI version, server version, the list of accepted authoring grammars (`dsl`), and the internal store schema version (`data`). If the server accepts no grammars, `dsl` reports `none`. If the server omits a field entirely, it reports `not reported`. Offline sessions omit `dsl` and `data` because only a running server can report them.
...
Scaffold a source, pipeline, transform, view, or serve resource into the workspace:
...
| Option | Purpose |
|---|---|
| `<kind>` | Positional argument selecting the resource kind: `source`, `pipeline`, `transform`, `view`, or `serve`. |
| `--connector <id>`, `-c` | Set the connector for a source resource. |
| `--id <id>` | Set the resource ID. |
| `--type <type>` | Set a transform type: `filter`, `map`, `js`, `union`, `nest`, or `join`. Use with `add transform`. |
| `--mode <mode>`, `-m` | Set a source read mode. |
| `--set key=value` | Set a connector configuration field; repeat for multiple fields. |
...
| `-w`, `--workdir <dir>` | Set the workspace directory. |

`new --kind <kind>` remains supported as a deprecated compatibility alias for `add <kind>` and prints a migration advisory. Bare `tapstate new` without arguments launches the guided interactive project initializer.
...
| `register <path\|connector-id> [-o text\|json\|yaml]` | Download and register an official connector by published ID, upload one connector JAR, or upload all direct `*.jar` children of a directory. Requires administrator authentication (`login admin`). |

`apply` reports each resource as created, updated, or unchanged. Connector registration is idempotent for the same artifact.
When an official connector ID is passed to `register`, the CLI resolves and downloads the certified artifact from GitHub releases (or a mirror specified by `TAPSTATE_CONNECTORS_URL`). If the download fails, the CLI returns `cli.connector-download-failed`.
...
| `stop <pipeline-id> [--keep-state] [-y]` | Stop a pipeline. Clears checkpoint state by default; pass `--keep-state` to preserve it. |
| `restart <pipeline-id> [--rerun] [-y]` | Restart a pipeline from its last checkpoint, or rerun from the beginning with `--rerun`. |
| `pause <pipeline-id>` | Pause a running pipeline. |
| `resume <pipeline-id>` | Resume a paused pipeline. |
| `position <pipeline-id> [-f <file>]` | Read or update checkpoint resume positions for a stopped pipeline. |
| `derived-schema <pipeline-id> [--accept]` | Compare schema evolution across pipeline steps, or adopt derived columns with `--accept`. |
| `status <pipeline-id>` | Read current pipeline lifecycle state, failure details when failed, and latest observation instant (`observedAt`). |
| `status <pipeline-id> --watch` | Stream lifecycle changes until interrupted. |
| `metrics <pipeline-id>` | Read current pipeline and per-table metrics or confirmed target positions. |
| `logs <pipeline-id>` | Read recent node-local logs, including forwarded connector and PDK runtime warnings and errors. |
| `logs <pipeline-id> --follow` | Stream node-local logs and forwarded connector logs until interrupted. |
...
`stop` clears checkpoint positions and internal operator state by default. When run in an interactive terminal, the CLI prints an inventory of state to be removed and prompts for confirmation (`Clear <id>? Type yes to go ahead`). To automate stopping and clearing in scripts, pass `-y` (or `--non-interactive`). To stop a pipeline while keeping its resume positions intact so a subsequent start resumes where it left off, pass `--keep-state`:

```bash
tapstate stop orders_pipeline --keep-state
```

`restart` manages the lifecycle sequence directly:

- **Default restart**: Safely cycles the pipeline without losing state. It pauses the running pipeline, waits until the engine confirms `PAUSED`, and resumes execution. If the pipeline is already stopped, it starts it.
- **Rerun from the beginning (`--rerun`)**: Stops the pipeline, clears this pipeline's checkpoint position and operator state (prompting for confirmation unless `-y` is passed), and starts a fresh run. On a dedicated source, the next run reads from the source beginning; on a shared mining chain where other pipelines are still running, the physical chain and its initial-load records are preserved:

```bash
tapstate restart orders_pipeline --rerun -y
```

### Inspect and adjust checkpoint positions

`position <pipeline-id>` reports the resume token and recording timestamp for each physical mining chain feeding the pipeline:

- **Positions belong to mining chains, not tables**: A single mining chain streams CDC events for all tables selected on a database connection. Offsets are tracked per chain rather than per individual table.
- **Chains can be shared across pipelines**: Multiple pipelines reading from the same physical database share the same mining chain. Changing a chain's position affects every pipeline reading through that chain; the CLI reports any other pipelines sharing the chain when a write-back occurs.

To adjust or rewind a resume position:

1. Read the current position payload:
   ```bash
   tapstate position orders_pipeline > position.json
   ```
2. Edit `resumeFrom.token` in `position.json`.
3. Apply the updated position while the pipeline is stopped:
   ```bash
   tapstate position orders_pipeline -f position.json
   ```

Modifying position while a pipeline or any pipeline sharing its mining chains is active is refused with `position.write-back-while-live`.

### Synchronize schema changes with `derived-schema`

Tapstate locks each pipeline run to a frozen copy of discovered source schemas so upstream DDL changes cannot unexpectedly disrupt an in-flight job.

Run `derived-schema <pipeline-id>` to inspect schema drift. It displays a three-way comparison for each pipeline step:

- **recorded**: The schema previously recorded for that step (omitted for uninferrable steps such as custom scripts).
- **derived**: The schema calculated now by applying pipeline transforms and joins to current stored discovery models.
- **target**: The column layout declared on the destination table.

When upstream database schemas change and the target is ready to accept the new structure:

1. Re-discover the source database so tapstate updates stored discovery models:
   ```bash
   tapstate discover-schema <source-id>
   ```
2. Stop the pipeline and inspect the drift:
   ```bash
   tapstate derived-schema orders_pipeline
   ```
3. Synchronize the pipeline definition:
   ```bash
   tapstate derived-schema orders_pipeline --accept
   ```

`--accept` copies the updated models from discovery storage into the pipeline definition, recomputes derived columns, and locks future pipeline starts to the updated schema. Running `--accept` while a pipeline is running is refused with `actuation.schema-sync-while-running`.
...
In a connected session, structured output is available for `delete`, `token`, `test`, `test-result`, `discover-schema`, `schema`, `register`, and `connectors`. Calling `position` without `-f` outputs structured JSON directly. Lifecycle, status, metrics, logs, and apply commands use their command-specific text output.
...
Automation should check the exit code instead of matching human-readable output. For multi-command scripted REPL sessions, the process exits with the exit code of the first rejected command rather than the final command executed.
